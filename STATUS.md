...
- added canonical `/api/v1` proof submission, contest handling, release/slash execution, party-history, party-score, lightweight status polling, first cron resolver routes, a party-history rebuild route, verifier-rule listing, first operator-route auth enforcement, and signed execution-payload validation
...
## Immediate next move
Verify execution signatures cryptographically, then keep tightening verifier-role attribution semantics.
